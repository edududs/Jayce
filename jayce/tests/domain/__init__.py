"""Tests for the domain routing policy."""
...
class TestRoutingPolicyDirectRoute:
    """Tests for messages that should route to direct_llm (no tools)."""
...
class TestRoutingPolicyToolRoute:
    """Tests for messages that should route to call_llm (with tools)."""
    ...
    def test_last_message_not_human(self) -> None:
        state = State(messages=[AIMessage(content="some response")])
        assert RoutingPolicy.should_route_direct(state) is False
